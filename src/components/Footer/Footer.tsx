import { Link } from 'react-router-dom';
import styles from './Footer.module.css';

export default function Footer() {
  return (
    <footer className={styles.footer}>
      <div className={styles.container}>
        <div className={styles.brandSection}>
          <div className={styles.logoRow}>
            <img src="/logo.svg" alt="Dillon & Bird" width={28} height={28} />
            <span className={styles.brandName}>Dillon & Bird</span>
          </div>
          <p className={styles.description}>
            Dillon & Bird Partners is a UAE-based strategy consulting firm advising founders and enterprises on business strategy, market entry, structuring, and scalable growth across the GCC.
          </p>
          <p className={styles.address}>
            Address: Meydan Hotel, Grandstand Building, 6th Floor, Al Meydan Road, Nad Al Sheba 1, Dubai, UAE
          </p>
          <div className={styles.socials}>
            <img src="/linkedin.svg" alt="LinkedIn" width={18} height={18} />
            <img src="/instagram.svg" alt="Instagram" width={18} height={18} />
          </div>
        </div>

        <div className={styles.linksSection}>
          <h4 className={styles.sectionTitle}>Our Services</h4>
          <ul>
            <li><Link to="/partnership">Partnerships</Link></li>
            <li><Link to="/managementconsulting">Management Consulting</Link></li>
            <li><Link to="/corporateservices">Corporate Services</Link></li>
            <li><Link to="/businesssetup">Business Site</Link></li>
            <li><Link to="/accountingfinancialservices">Accounting & Financial Services</Link></li>
            <li><Link to="/financialplanninginvestorservices">Financial Planning & Investor Services</Link></li>
            <li><Link to="/technologyconsulting">Technology Consulting</Link></li>
          </ul>
        </div>

        <div className={styles.linksSection}>
          <h4 className={styles.sectionTitle}>Company</h4>
          <ul>
            <li><Link to="/about">Who We Are</Link></li>
            <li><Link to="/casestudies/technologystartup">Technology Startup</Link></li>
            <li><Link to="/casestudies/retailbusiness">Retail Business</Link></li>
            <li><Link to="/contact">Contact</Link></li>
          </ul>
        </div>

        <div className={styles.newsletter}>
          <div className={styles.inputRow}>
            <input type="text" placeholder="subscribe to our news letter" />
            <button>Subscribe</button>
          </div>
          <p className={styles.note}>
            Stay updated with the latest insights and opportunities from our team.
          </p>
        </div>
      </div>

      <div className={styles.bottomBar}>
        <div className={styles.bottomContainer}>
          <span>© 2026. Dillon and Bird Consulting Partners. All rights reserved.</span>
          <div className={styles.bottomLinks}>
            <Link to="/privacy-policy">Privacy Policy</Link>
            <span>|</span>
            <Link to="/terms">Terms & Conditions</Link>
            <span>|</span>
            <Link to="/cookies">Cookie Policy</Link>
          </div>
        </div>
      </div>
    </footer>
  );
}